...
namespace PLAYERTWO.PlatformerProject
{
	public abstract class Entity : MonoBehaviour
	{
		...
		public virtual bool OnSlopingGround() =>
			isGrounded
			&& !platform
			&& Vector3.Angle(transform.up, CurrentWorldUp()) > m_slopingGroundAngle;
		...
		where T : Entity<T>
	{
		...
		protected virtual void HandleGround()
		{
			...
			var validAngle = colliding && Vector3.Angle(normal, worldUp) <= controller.slopeLimit;
			var landing =
				colliding && movingTowardGround && (isGrounded && rotateToGround || validAngle);
		...
		protected virtual void UpdateGroundRotation()
		{
			...
			if (
				gravityField
				|| onRails
				|| (!rotateToGround && !rotateToPlatforms)
				|| !HasGroundBelow(out var hit)
			)
				return;

			var speed = lateralVelocity.magnitude;
			var normal = isGrounded ? hit.normal : CurrentWorldUp();
			var rotation = Quaternion.FromToRotation(transform.up, normal);
		...
		public virtual void SlopeFactor(float upwardForce, float downwardForce)
		{
			...
			var worldUp = CurrentWorldUp();
			var horizontalNormal = Vector3.ProjectOnPlane(transform.up, worldUp);
			...
			velocity += factor * multiplier * Time.deltaTime * -worldUp;
		}
